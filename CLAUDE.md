...
- **Completion / multi-step (designed, measured, TO BUILD):** the LLM CANNOT plan/decompose (spurious `complete` even handed explicit progress, §2.14) → **deterministic sequencer**: split goal → ordered sub-goals (parseable-sequential only; un-parseable → handback); per-sub-goal **expected-effect signatures** (action-class enum) used as precondition (already-satisfied → advance w/o acting) AND postcondition (class effect occurred → advance) — NOT bare change-diff; **deviation→escalate** (screen matches neither → handback, since the deterministic plan is blind/can't re-plan). NO LLM planner. NO Board memory anywhere in the loop (inv #10).
