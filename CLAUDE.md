...
- **Completion / multi-step (CORE BUILT + LIVE, commit 37587c2; refinements pending):** the LLM CANNOT plan/decompose (spurious `complete` even handed explicit progress, §2.14) → **deterministic sequencer**. BUILT: `decompose_goal` (split on explicit sequential markers; semantic-compound stays one sub-goal — no mangled plan) + `sub_goals`/`current_sub` pointer + pointer ADVANCES on action-effect (structural a11y screen change) + deterministic completion when the plan is exhausted. **2-step VM walk passes** (open menu → launch terminal, 2 clicks). NO LLM planner; NO Board memory in the loop (inv #10). PENDING REFINEMENTS (§2.15, documented not hidden): per-action-class **effect signatures** + **precondition skip** (already-satisfied → advance w/o acting) + **deviation→escalate** (screen matches neither precondition nor post-effect → handback, since the deterministic plan can't re-plan). v1 advance = structural change, which is the simpler proxy.
